...
$('#osInsumo').on('show.bs.select', function (e, clickedIndex, isSelected, previousValue) {
    console.log('Estoy dentro del evento loaded.bs.select');
...
/*
 * Al mostrar 'osInsumo', configura el panel de multiselección al centro de la pantalla
 *
 * */
$('#osInsumo').on('shown.bs.select', function (e, clickedIndex, isSelected, previousValue) {
    let $style = '';
    var $regexminwidth = /min-width/i;
    let $maxtop = document.querySelector('header').offsetHeight; //document.querySelector('nav.navbar.navbar-expand-xl.navbar-dark.bg-dark.sticky-top').offsetHeight;
    var $regextop = /top/i;
    Array.prototype.forEach.call(document.querySelector('div.dropdown-menu.show').attributes.getNamedItem('style').value.split(';'), (c, i, l) => {
        if ($regextop.exec(c) !== null) {
            $style += ' top: ' + document.querySelector('header').offsetHeight /*document.querySelector('nav.navbar.navbar-expand-xl.navbar-dark.bg-dark.sticky-top').offsetHeight*/ + 'px;';
...
function numeroTelefonico(event) {
    if (Math.abs(event.srcElement.value) != 'NaN') {
        event.srcElement.value = Math.abs(event.srcElement.value);
    }
}

// *********************  criterio de fecha con relación a línea de negocio ***********************
/*document.getElementById('iLineaNegocio').addEventListener('change', e => {
  if(e.srcElement.options.length > 1){
    ...
    //if(document.getElementById('iFechaFin').querySelector('input').value != ''){
      ...
      $('#iFechaFin').datetimepicker('destroy');
    //}
    if(e.srcElement.options.item(e.srcElement.options.selectedIndex).innerText == 'INSTALACIÓN'){
    ...
    } else if(e.srcElement.options.item(e.srcElement.options.selectedIndex).innerText == 'MANTENIMIENTO') {
      $('#iFechaInicio').datetimepicker({
    ...
    } else if(e.srcElement.options.item(e.srcElement.options.selectedIndex).innerText == 'FLETE') {
      $('#iFechaInicio').datetimepicker({
        minDate: moment().add(3, 'd'),
        defaultDate: moment().add(3,'d'),
        disabledDates: [
...
$(document).ready(function () {
    let iOp3 = document.getElementById('iOp3');
    if (iOp3 != null)
        document.getElementById('btnSave').addEventListener('click', e => {
            if (iOp3.value == '')
                iOp3.value = 'Por definir';
        });
    ...
    document.getElementById('iOp2').addEventListener('change', e => {
        ...
        $.ajax({
            ...
            success: function (d, s, x) {
                if (!d) {
                    if (e.srcElement.attributes.getNamedItem('data-toggle') == null) {
                        ...
                        e.srcElement.setAttribute('data-content', 'La información capturada se utilizará para realizar la búsqueda de una Orden de Servicio e intentar vincularla a esta misma');
            ...
            error: function (request, typeError, exceptionObject) {
                console.log({ request }, { typeError }, { exceptionObject });
            }
        });
    });
    ...
    //******************** criterio de fecha según línea de negocio *************************
    /*document.getElementById('iFechaInicio').addEventListener('change', e => {
      var regex = /[0-3][0-9]\/[01][0-9]\/[2][0][0-9][0-9]/gm;
      const inputDate = document.getElementById('iFechaInicio').querySelector('input').value;
      if(regex.exec(inputDate) != null){
        const iDarray = inputDate.split('/');
        if(!(iDarray[0] > 0 && iDarray[0] <= 31)){
          $('#iFechaInicio').datetimepicker('destroy');
          $('#iFechaInicio').datetimepicker('destroy');
          $('#iFechaInicio').datetimepicker({
            locale: 'es',
          ...
          document.getElementById('iFechaInicio').querySelector('input').focus();
          return;
        }
        if(!(iDarray[1] > 0 && iDarray[1] <= 12)){
          $('#iFechaInicio').datetimepicker('destroy');
          $('#iFechaInicio').datetimepicker('destroy');
          $('#iFechaInicio').datetimepicker({
            locale: 'es',
        ...
        if(!(moment(iDarray[2],'YYYY').isSameOrAfter(moment().format('YYYY')) && moment(iDarray[2],'YYYY').isSameOrBefore(moment().add(1, 'y')))){
          $('#iFechaInicio').datetimepicker('destroy');
          $('#iFechaInicio').datetimepicker('destroy');
          $('#iFechaInicio').datetimepicker({
            locale: 'es',
        ...
        $.ajax({
          url: '/Rest/GetValidations',
          success: function(data, status, jqXHR){
            const d = data.split(' ');
            const cV = document.getElementById('iLineaNegocio').options.item(document.getElementById('iLineaNegocio').options.selectedIndex).innerText;
            const iD = moment(iDarray[2]+'-'+iDarray[1]+'-'+iDarray[0], 'YYYY-MM-DD');
            for (let i = 0; i < d.length/2; i++){
              if(d[i*2] == cV){
                if( iD.isSameOrBefore(moment().add(d[(i*2)+1], 'days'))){
                  $('#iFechaInicio').datetimepicker('destroy');
                  $('#iFechaInicio').datetimepicker('destroy');
                  $('#iFechaInicio').datetimepicker({
                    minDate: moment().add(d[(i*2)+1], 'd'),
                    defaultDate: moment().add(d[(i*2)+1],'d'),
                    disabledDates: disabledDatesArray(d[(i*2)+1]),
                    locale: 'es',
          focusOnShow: false,
                    //format: 'L'
                    inline: true,
    ...
    // ****************  Criterio de fechas según línea de negocio ********************
    /*document.getElementById('iFechaFin').addEventListener('change', e => {
      var regex = /[0-3][0-9]\/[01][0-9]\/[2][0][0-9][0-9]/gm;
      const inputDate = document.getElementById('iFechaFin').querySelector('input').value;
      if(regex.exec(inputDate) != null){
        const iDarray = inputDate.split('/');
        if(!(iDarray[0] > 0 && iDarray[0] <= 31)){
          $('#iFechaFin').datetimepicker('destroy');
          ...
          document.getElementById('iFechaFin').querySelector('input').focus();
          return;
        }
        if(!(iDarray[1] > 0 && iDarray[1] <= 12)){
          $('#iFechaFin').datetimepicker('destroy');
        ...
        $.ajax({
          ...
          success: function(data, status, jqXHR){
            const d = data.split(' ');
            const cV = document.getElementById('iLineaNegocio').options.item(document.getElementById('iLineaNegocio').options.selectedIndex).innerText;
            const iD = moment(iDarray[iDarray.length-1]+iDarray[iDarray.length-2]+iDarray[iDarray.length-3], 'YYYYMMDD');
            for (let i = 0; i < d.length/2; i++){
              if(d[i*2] == cV){
                if( iD.isSameOrBefore(moment().add(d[(i*2)+1], 'days'))){
                  $('#iFechaFin').datetimepicker('destroy');
    ...
    $('#iFechaFin').on('change.datetimepicker', function (e) {
    ...
    $('#iFechaInicio').on('change.datetimepicker', function (e) {
        var fin = document.getElementById('iFechaFin').querySelector('input').value.split(' ')[0].split('/')[2] + '-' + document.getElementById('iFechaFin').querySelector('input').value.split(' ')[0].split('/')[1] + '-' + document.getElementById('iFechaFin').querySelector('input').value.split(' ')[0].split('/')[0] + ' ' + document.getElementById('iFechaFin').querySelector('input').value.split(' ')[1];
        var inicio = document.getElementById('iFechaInicio').querySelector('input').value.split(' ')[0].split('/')[2] + '-' + document.getElementById('iFechaInicio').querySelector('input').value.split(' ')[0].split('/')[1] + '-' + document.getElementById('iFechaInicio').querySelector('input').value.split(' ')[0].split('/')[0] + ' ' + document.getElementById('iFechaInicio').querySelector('input').value.split(' ')[1];
        if (moment(fin).isBefore(moment(inicio))) {
            $('#fechaFinMenorFechaInicio').modal('show');
    ...
    function disabledDatesArray(count) {
        var days = [];
        for (let i = 0; i <= count; i++) {
            days.push(moment().add(i, 'd'));
        }
        return days;
    }
        ...
        $('#fNewOS').parsley().whenValidate({
            ...
            if (document.getElementById('iLineaNegocio').options.item(document.getElementById('iLineaNegocio').options.selectedIndex).innerText != 'GARANTÍA') {
                ...
                $.ajax({
                    ...
                    data: {
                        'cId': parseInt(document.getElementById('iCliente').options.item(document.getElementById('iCliente').options.selectedIndex).value),
                    ...
                    success: function (d, s, jqXHR) {
                        ...
                        th.setAttribute('data-field', 'os');
                        th.setAttribute('style', '');
                        let thdiv = document.createElement('div');
                        thdiv.classList.add('th-inner');
                        thdiv.innerText = 'Orden Servicio';
                        th.appendChild(thdiv);
                        let thdiv2 = document.createElement('div');
                        thdiv2.classList.add('fht-cell');
                        th.appendChild(thdiv2);
                        let thr = document.getElementById('tActivoFijo').querySelector('th').parentElement;
                        thr.insertBefore(th, thr.children[3]);
            
                        //
                        //  Continúa el cuerpo de la tabla 'tActivoFijo'
                        //
                        //$('#tActivoFijo').bootstrapTable('load', '');
                        */
        ...
        $.ajax({
            ...
            success: function (data, status, jqXHR) {
                if (data !== null) {
                    ...
                    $('#iContrato').empty().append('<option value=""> -- Selecciona una opci&oacute;n -- </option>');
                    $.each(_list, function (idx, d) {
        ...
        $.ajax({
            ...
            success: function (data, status, jqXHR) {
                if (data !== null) {
                    ...
                    _list = _list.sort((a, b) => (a.Nombre > b.Nombre) ? 1 : -1);
                    $('#iUbicacion').empty().append('<option value=""> -- Selecciona una opci&oacute;n -- </option>');
                    $.each(_list, function (idx, d) {
    ...
    $('#btnAddPersonal').click(function () {
        ...
        if (!duplicidad) {
            ...
            if (sPersona.val() > 0) {
                ...
                data.actions = `<button id=idx_${tIdx + 1} type="button" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>`;
    ...
    $(document).on('click', '#tPersonal :button', function (e) {
        ...
        $.each(befDelete, function (idx, d) {
            if (String(d["id"]) !== String(ids)) {
                ...
                p.nombre = d.nombre;
                p.puesto = d.puesto;
                p.actions = `<button id=idx_${aidx + 1} type="button" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>`;
                data.push(p);
                aidx++;
            }
        });
    ...
    $('#btnAddInsumo').click(function () {
        ...
        if (parseInt(document.getElementById('os_Insumo').options.item(document.getElementById('os_Insumo').options.selectedIndex).value) != NaN && document.getElementById('osCantidad').value == '') {
            ...
            if (sInsumo.val() > 0) {
                ...
                data.actions = `
        <button id=idx_${tIdx + 1} type="button" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
        <button id="Edit_${tIdx + 1}" type="button" class="btn btn-success btn-sm"><i class="fas fa-edit"></i></button>
        `;
    ...
    $('#fNewOS').submit(function () {
        $("[name='Estatus']").val(true);
