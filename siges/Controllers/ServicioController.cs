      ...
      if(User.Identity.IsAuthenticated){
        if(osId != null && osId > 0){
          ...
          try{
            ...
            ((System.Data.SqlClient.SqlConnection)conn).Open();
            cmd = new SqlCommand("select ar.exifbi from archivo ar left join estado es on ar.estadoid = es.id left join operador op on es.operadorid = op.id left join ordenservicio os on op.ordenservicioid = os.id where es.nuevoestado = 'atendido' and ar.exifbi is not null and op.ordenservicioid = @Id", (System.Data.SqlClient.SqlConnection)conn);
            cmd.Parameters.Add("@Id", SqlDbType.Char);
            cmd.Parameters["@Id"].Value = osId;
            ...
            ((System.Data.SqlClient.SqlConnection)conn).Open();
            cmd = new SqlCommand("select os.FechaInicio, os.FechaFin, ub.direccion, ub.latitude, ub.longitude, ub.distancia from ordenservicio os left join ubicacion ub on os.ubicacionid = ub.id where os.id = @Id", (System.Data.SqlClient.SqlConnection)conn);
            cmd.Parameters.Add("@Id", SqlDbType.Char);
            cmd.Parameters["@Id"].Value = osId;
            dataReader = cmd.ExecuteReader();
            if(dataReader.HasRows){
            ...
            ((System.Data.SqlClient.SqlConnection)conn).Open();
            cmd = new SqlCommand("select ar.[file], es.comentarionuevoestado from archivo ar left join estado es on ar.estadoid = es.id left join operador op on es.operadorid = op.id left join ordenservicio os on op.ordenservicioid = os.id where es.nuevoestado = 'sitio' and ar.[file] is not null and op.ordenservicioid = @Id", (System.Data.SqlClient.SqlConnection)conn);
            cmd.Parameters.Add("@Id", SqlDbType.Char);
            cmd.Parameters["@Id"].Value = osId;
            dataReader = cmd.ExecuteReader();
            if(dataReader.HasRows){
      ...
      if(this.User.IsInRole("Operador")){
        ...
        ViewData["OrdenServicioEnTiempo"] = JsonConvert.SerializeObject(getOSenTiempoOperador().Result, jsonSettings);
          ...
          ViewData["ordenes"] = _all;
            ...
            _all = new List<OrdenServicio>();
            foreach(OrdenServicio oss in _osRepo.GetAll(true).OrderByDescending(r => r.Id).ToList()){
      ...
      if((bool)settings.UsoDePaquetes){
        ...
        if (insumos.Count > 0) {
          foreach (var x in insumos) {
            os.Insumos.Add(new OrdenInsumo() {
                Insumo = _iRepo.GetById(x.InsumoId),
                Cantidad = x.Cantidad, Estatus = true 
                });
          }
        }
      }
