...
namespace siges.Controllers
{
    ...
    public class RestController : Controller
    {
        ...
        {
            Console.WriteLine("\n\n\ninicio GETCONTACTOCLIENTE con id: " + cId + "\n\n\n");
            if (cId > 0)
            {
        ...
        */
        ...
        public JsonResult GetContactoCliente(int cId)
        {
            if (cId > 0)
            {
                //IQueryable<ContactoCliente> cc = _context.ContactoCliente.Where(r => r.Cliente.Id == cId).Include(r => r.Contactos.Where(s => s.Estatus == true));
                IQueryable<ContactoCliente> cc = _context.ContactoCliente.Where(r => r.Cliente.Id == cId).Include(r => r.Cliente).Include(r => r.Contactos);
                try
                {
                    if (cc.Any())
                    {
        ...
        public JsonResult GetPersonas()
        {
            ...
            Microsoft.Data.SqlClient.SqlCommand cmd = new Microsoft.Data.SqlClient.SqlCommand("select per.id from aspnetuserroles rol left join aspnetusers usr on rol.userid = usr.id left join persona per on usr.perid = per.id left join aspnetroles rls on rol.roleid = rls.id where rls.name = @Role", (Microsoft.Data.SqlClient.SqlConnection)conn);
            cmd.Parameters.Add("@Role", SqlDbType.NVarChar);
            cmd.Parameters["@Role"].Value = "Operador";
            Microsoft.Data.SqlClient.SqlDataReader dataReader = cmd.ExecuteReader();
